...
**招待リンク** : {invite.url}""",
                colour=discord.Colour.green()
            ).set_thumbnail(url=server_icon), content="<@&1503764104483967237>")
...
**招待リンク** : {invite.url}""",
    ...
    @tagdb.command(name="category", description="アクセス範囲についてを変更します。")
    @app_commands.describe(
        id="管理用ID",
        kind="アクセス範囲"
    )
    async def category(self, interaction:discord.Interaction, id:int, kind:Literal["標準", "参加申請"]):
        await interaction.response.defer()
        try:
            db:Tags = await self.DB.get_tag(id=id)
            if db.count == 0:
            ...
            await interaction.followup.send(embed=discord.Embed(
                title="カテゴリ変更",
                description="アクセス範囲を変更し、サーバー名とアイコンも更新しました。",
                colour=discord.Colour.green()
            ))
        except NotFound as e:
            await interaction.followup.send(embed=discord.Embed(
